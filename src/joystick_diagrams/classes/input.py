...
class Input:
    ...
    def add_modifier(self, modifier: set, command: Command):
        ...
        _logger.warning(f"Existing modifier check is {existing}")
    ...
    def _check_existing_modifier(self, modifier: set) -> Modifier | None:
        ...
        _logger.warning(f"Checking for existing modifier {modifier}")
        _logger.warning(f"Existing modifiers: {self.modifiers}")
...
class LogicalDevice:
    ...
    def add_modifier_to_input(self, input_id, modifier: set, command: Command) -> None:
        ...
        _logger.warning(f"Adding modifier {modifier} to input {input_id}")
        obj = self._get_input(input_id)
        _logger.warning(f"Modifier input is: {obj}")
        if obj is None:
            self.create_input(input_id, InputTypes.BUTTON, "Not Used", [])
            obj = self._get_input(input_id)
            _logger.warning(f"Modifier input created now: {obj}")
