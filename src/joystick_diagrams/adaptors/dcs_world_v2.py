...
class DCSWorldParser(jdi.JDinterface):
    ...
    def get_validated_profiles(self) -> list[str]:
        ...
        if self.remove_easy_modes:
            return list(
                filter(
                    lambda x: False if EASY_MODE in x else True,
                    self.valid_profiles,
                )
            )
    ...
    @staticmethod
    def extract_device_details(file_name: str) -> tuple:
        return (file_name[-46:-10], file_name[:-48])  # Guid  # Name
    ...
    def process_profiles(self, profile_list: list) -> dict[str]:
        ...
        for profile in profile_list:
            _logger.warning(f"Profile: {profile}")
            fq_path = os.path.join(self.path, "Config", "Input", profile, "joystick")
            profile_device_files = os.listdir(os.path.join(fq_path))

            _logger.warning(f"Profile paths: {profile_device_files}")

            for file in profile_device_files:

                _logger.warning(f"Processing file: {file}")
                guid, name = self.extract_device_details(file)

                ipt.add_device(guid=guid, name=name)

                if os.path.isdir(os.path.join(fq_path, file)):
                    print("Skipping as Folder")
                else:
                    try:
                        self.file = Path(os.path.join(fq_path, file)).read_text(encoding="utf-8")
                        self.file = self.file.replace("local diff = ", "")  ## CLEAN UP
                        self.file = self.file.replace("return diff", "")  ## CLEAN UP
                    except FileNotFoundError:
                        raise FileExistsError(
                            "DCS: File {} no longer found - It has been moved/deleted from directory".format(file)
                        )
                    else:
                        file_dictionary = self.parse_file()

                        # Merge outputs into master dictionary in profile order
                        if "axisDiffs" in file_dictionary.keys():
                            final_dict["axisDiffs"] |= file_dictionary["axisDiffs"]

                        if "keyDiffs" in file_dictionary.keys():
                            final_dict["keyDiffs"] |= file_dictionary["keyDiffs"]

        return final_dict

    def create_joystick_map(self, data: dict, guid: str) -> dict:
        ...
        if "keyDiffs" in data.keys():
            for value in data["keyDiffs"].values():
                ...
                for item, attribute in value.items():
                    ...
                    if button & name:
                        ipt.add_inputs(guid, input_id=button, style=ipt.InputTypes.BUTTON, command=name, modifiers=[])
