...
class CustomiseProfiles(QWidget):
    """Sub-screen for profile browsing, bind inspection, and custom label management."""

    device_visibility_changed = Signal()
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        # Build the unified layout: left panel (profiles) | right panel (binds + labels)
        ...
        # === RIGHT PANEL: Search + Binds tree + Custom Labels ===
        right_panel = QWidget()
        ...
        # Binds tree widget — built programmatically (no designer file)
        self.viewBindsTreeWidget = QTreeWidget()
    ...
    def refresh(self):
        """Reload profiles, binds, and custom labels."""
        self.initialise_available_profiles()
        self._load_first_profile()
    ...
    def _load_binds_for_profile(self, profile_wrapper: ProfileWrapper):
        ...
        for device_obj in profile_data.get_devices().values():
            ...
            display_name = self.appState.device_service.resolve_name(
                device_obj.guid, device_obj.name
            )

            device_root = QTreeWidgetItem(self.viewBindsTreeWidget)
            device_root.setText(0, display_name)
            ...
            if not device_inputs:
                device_root.setText(0, f"{display_name}  —  No inputs")
    ...
    def show_context_menu(self, position):
        ...
        if original is None and item.parent() is None:
            ...
            if action == hide_action:
                ...
                self.device_visibility_changed.emit()
            return
