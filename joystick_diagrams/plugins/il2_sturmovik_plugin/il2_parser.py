...
class IL2Parser:
    ...
    def process_profiles(self) -> ProfileCollection:
        ...
        try:
            ...
            _logger.info(
                f"Files parsed successfully. Found {len(self.devices)} devices and {len(self.bindings)} bindings"
            )
            self._create_profiles(profile_collection)
            _logger.info(
                f"Profile creation completed. Created {len(profile_collection.profiles)} profiles"
            )
            ...
            for encoding in encodings_to_try:
                try:
                    with open(self.devices_file, "r", encoding=encoding) as file:
                        content = file.read()
                    _logger.info(
                        f"Devices file read successfully with {encoding} encoding. Content length: {len(content)} characters"
                    )
                    break
                except UnicodeDecodeError:
                    _logger.debug(
                        f"Failed to read devices file with {encoding} encoding, trying next..."
                    )
                    continue
    ...
    def _parse_global_actions_file(self):
        ...
        try:
            ...
            for encoding in encodings_to_try:
                try:
                    with open(self.global_actions_file, "r", encoding=encoding) as file:
                        content = file.read()
                    _logger.info(
                        f"Global.actions file read successfully with {encoding} encoding. Content length: {len(content)} characters"
                    )
                    break
                except UnicodeDecodeError:
                    _logger.debug(
                        f"Failed to read global.actions with {encoding} encoding, trying next..."
                    )
                    continue
            ...
            _logger.error(
                f"Error parsing global.actions file {self.global_actions_file}: {e}"
            )
            raise
    ...
    def _fix_encoding_issues(self, content: str) -> str:
        ...
        try:
            ...
            _logger.debug(
                f"Byte-level encoding fix failed: {e}, trying string replacements"
            )
    ...
    def _extract_device_info(self, content: str):
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            try:
                ...
                _logger.debug(
                    f"Found device: {model} (joy{config_id}) with GUID: {device_uuid}"
                )

            except Exception as e:
                _logger.warning(
                    f"Failed to parse device line {line_num}: {stripped_line} - {e}"
                )
                continue
                ...
                if comment_part.startswith("//"):
                    ...
                    if description:
                        ...
                        if len(parts) >= 1:
                            ...
                            _logger.debug(
                                f"Found description for '{action_name}': '{description}'"
                            )

    def _parse_binding_line(
    ...
    def _parse_il2_device_reference(self, device_ref: str) -> Optional[Dict]:
        ...
        if button_match:
            ...
            return {"type": "button", "device_id": device_id, "button_id": button_id}
        ...
        if axis_match:
            ...
            return {"type": "axis", "device_id": device_id, "axis_id": axis_id}
    ...
    def _create_profiles(self, profile_collection: ProfileCollection):
        ...
        for device_id, bindings in device_bindings.items():
            ...
            if device_info:
                ...
                _logger.warning(
                    f"No device mapping found for joy{device_id} in devices.txt. Available device IDs: {list(self.devices.keys())}"
                )
            ...
            for binding in bindings:
                ...
                if control:
                    ...
                    if existing_input:
                        ...
                        if (
                            existing_input.command
                            and existing_input.command != display_name
                        ):
                            existing_input.command = (
                                f"{existing_input.command} | {display_name}"
                            )
                            _logger.debug(
                                f"Combined commands for {control.identifier}: {existing_input.command}"
                            )
                        elif not existing_input.command:
                            existing_input.command = display_name
                            _logger.debug(
                                f"Set command for existing input {control.identifier}: {display_name}"
                            )
                    else:
                        # Create new input
                        device.create_input(control, display_name)
                        _logger.debug(
                            f"Created input for {display_name} → {control.identifier}"
                        )
                else:
                    _logger.warning(
                        f"Failed to create control object for binding: {binding}"
                    )
        ...
        try:
            if control_type == "button":
                ...
                if hat_match:
                    ...
                    _logger.warning(
                        f"Could not extract HAT ID from {hat_id_str}, using 1"
                    )
                    hat_id_numeric = 1
    ...
    def export_mapping_to_file(
        ...
        try:
            ...
            with open(output_file_path, "w", newline="", encoding="utf-8") as f:
                ...
                for item in sorted(
                    export_data, key=lambda x: (x["device_name"], x["real_button"])
                ):
                    writer.writerow(
                        {
            ...
            _logger.info(
                f"Successfully exported {len(export_data)} mappings to CSV file {output_file_path}"
            )
            return True

        except Exception as e:
            _logger.error(f"Failed to export mappings to CSV file: {e}", exc_info=True)
            return False
    ...
    def _binding_matches_input(self, binding: dict, input_item) -> bool:
        ...
        try:
            ...
            if input_type == "button" and binding_type == "button":
                ...
                input_id = getattr(input_item, "identifier", "")
                return binding.get("hat_id", "").replace("_", "") in input_id.replace(
                    "_", ""
                )
    ...
    def _get_device_id_from_guid(self, device_guid: str, device_name: str) -> str:
        ...
        for device_id, device_info in self.devices.items():
            if (
                device_info.get("guid") == device_guid
                or device_info.get("name") == device_name
            ):
                return device_id
